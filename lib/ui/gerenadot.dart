                          ...
                          trailing: IconButton(
                              ...
                              onPressed: () {
                                Navigator.push(
                                    ...
                                    MaterialPageRoute(
                                        builder: (context) =>
                                            ChatScreen(meusDocuments[index])));
                              }),
                          subtitle: Text(meusDocuments[index]["raca"]),
                          title: Text(
                            meusDocuments[index]["nome"],
                            style: TextStyle(fontSize: 20),
                          ),
                          leading: CircleAvatar(
                            backgroundImage:
                                NetworkImage(meusDocuments[index]["imgUrl"]),
                          ),
                        ),
                        onDismissed: (direction) async {
                          int ultimoIndex = index;

                          DocumentSnapshot ultimoReg =
                              meusDocuments[ultimoIndex];

                          CollectionReference pet =
                              await Firestore.instance.collection('pet');
                          await pet
                              .document(meusDocuments[ultimoIndex].documentID)
                              .delete();
                          setState(() {
                          ...
                          final snack = SnackBar(
                            content:
                                Text("Pet \"${ultimoReg["nome"]}\" removido"),
                            action: SnackBarAction(
                                label: "Desfazer",
                                onPressed: () {
                                  setState(() {
                                    meusDocuments.insert(
                                        ultimoIndex, ultimoReg);
                                  });
